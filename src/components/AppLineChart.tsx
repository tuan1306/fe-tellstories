...
export function ChartAreaInteractive() {
  ...
  React.useEffect(() => {
    fetch("/api/dashboard")
      .then((res) => res.json())
      .then((data) => {
        const raw = data?.data?.statistics ?? [];
        const transformed = (raw as DashboardStat[]).map((item) => ({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return dashboardData.filter((item) => new Date(item.date) >= start);
    ...
    <Card className="pt-0">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription>
            Showing total accounts active for the last 3 months
          </CardDescription>
        </div>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          <AreaChart data={filteredData}>
            ...
            {/* I literally don't know how to fix this, it's library fault */}
